...
| `publicUrl` | No | URL | empty | To correctly set the `redirect_uri` for the OAuth2 callback, the middleware needs to know the URL under which the Portal-Gateway is accessible. This setting is also used to patch OIDC URLs, e.g. `authorization_endpoint` and `issuer`, to ensure all Keycloak related traffic is routed through the Gateway. Generally, the environment variables `PORTAL_GATEWAY_PUBLIC_PROTOCOL`, `PORTAL_GATEWAY_PUBLIC_HOSTNAME`, and `PORTAL_GATEWAY_PUBLIC_PORT` are used for this. However, these can be overridden per middleware with this property. This is necessary, if `oauth2` middlewares are used under different hosts. |
