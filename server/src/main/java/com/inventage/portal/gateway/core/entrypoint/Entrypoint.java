...
public class Entrypoint {
  ...
  public Router router() {
    if (router == null) {
      ...
      router.route()
          .handler(SessionHandler.create(LocalSessionStore.create(vertx)).setSessionCookieName(SESSION_COOKIE_NAME)
              .setCookieHttpOnlyFlag(SESSION_COOKIE_HTTP_ONLY).setCookieSecureFlag(SESSION_COOKIE_SECURE)
              .setCookieSameSite(SESSION_COOKIE_SAME_SITE).setMinLength(SESSION_COOKIE_MIN_LENGTH).setNagHttps(true));
    }
    return router;
  }
    ...
    optionApplicationRouter.ifPresent(applicationRouter -> {
      if (name.equals(application.entrypoint())) {
        if (enabled()) {
          ...
          LOGGER.warn("mount: disabled endpoint '{}' can not mount application '{}' for '{}'", name, application,
  ...
  public static JsonObject entrypointConfigByName(String name, JsonObject globalConfig) {
    ...
    return configs.stream().map(object -> new JsonObject(Json.encode(object)))
        .filter(entrypoint -> entrypoint.getString(StaticConfiguration.ENTRYPOINT_NAME).equals(name)).findFirst()
        .orElseThrow(() -> {
