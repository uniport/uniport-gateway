...
public class KubernetesServiceProvider extends Provider {
  ...
  public void provide(Promise<Void> startPromise) {
    ...
    this.kubernetesDiscovery.registerServiceImporter(new KubernetesServiceImporter(), new JsonObject().put("host", host)
        .put("port", port).put("ssl", useTLS).put("token", token).put("namespace", namespace));
