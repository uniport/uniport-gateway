...
public class DynamicConfiguration {
  ...
  private static Schema buildSchema(Vertx vertx) {
    ...
    ObjectSchemaBuilder serviceSchema = Schemas.objectSchema().requiredProperty(SERVICE_NAME, Schemas.stringSchema())
        .requiredProperty(SERVICE_SERVERS,
            Schemas.arraySchema()
                .items(Schemas.objectSchema().requiredProperty(SERVICE_SERVER_HOST, Schemas.stringSchema())
                    .requiredProperty(SERVICE_SERVER_PORT, Schemas.intSchema()).allowAdditionalProperties(false)))
        .allowAdditionalProperties(false);

    ObjectSchemaBuilder httpSchema = Schemas.objectSchema().property(ROUTERS, Schemas.arraySchema().items(routerSchema))
  ...
  public static Future<Void> validate(Vertx vertx, JsonObject json, boolean complete) {
    ...
    schema.validateAsync(json).onSuccess(f -> {
      ...
      List<Future> validFutures = Arrays.asList(validateRouters(httpConfig, complete), validateMiddlewares(httpConfig),
  ...
  private static Boolean addService(JsonObject httpConf, String serviceName, JsonObject serviceToAdd) {
    ...
    JsonObject existingService = getObjByKeyWithValue(existingServices, DynamicConfiguration.SERVICE_NAME, serviceName);
