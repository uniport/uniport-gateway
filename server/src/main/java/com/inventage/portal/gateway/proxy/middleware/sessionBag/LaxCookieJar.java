 ...
 * 
 * The cookie jar from vertx only support parsing cookies from the request cookie header in netty's strict mode
 ...
 * 
 * However, the session bag also needs to support "invalid" cookies e.g. containing whitespaces.
 * 
 * Due to vertx restricting the visibility of some key components, this class is hacked together.
 * - We cant use 'new CookieJar(cookieHeader)' because it is not visible
 * - We must cast blindly the 'Cookie' to 'ServerCookie' as the 'CookieJar' only supports these.
 * This is based on the assumption of 'CookieImpl implements ServerCookie' and 'ServerCookie extends Cookie'
 * 
...
public class LaxCookieJar extends CookieJar {
    ...
    public LaxCookieJar(CharSequence cookieHeader) {
        ...
        final Set<io.netty.handler.codec.http.cookie.Cookie> nettyCookies = ServerCookieDecoder.LAX.decode(cookieHeader.toString());
