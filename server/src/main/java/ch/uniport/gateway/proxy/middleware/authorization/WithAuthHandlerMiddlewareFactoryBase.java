...
import ch.uniport.gateway.proxy.middleware.authorization.bearerOnly.customIssuerChecker.JWTAuthMultipleIssuersOptions;
...
public abstract class WithAuthHandlerMiddlewareFactoryBase implements MiddlewareFactory {
            ...
            for (Object claim : claims.getList()) {
                ...
                if (cObj.size() != 3) {
                    return Future.failedFuture(
                        "Claim is required to contain exactly 3 entries. Namely: claimPath, operator and value");
                }
     ...
     *
     * @param authHandler
     *            The {@link AuthenticationHandler} to use in the
     *            middleware
     * @param middlewareConfig
     *            The config for the middleware
     * @return Your {@link Middleware}
     */
    protected abstract Middleware create(
    ...
    public Future<Middleware> create(Vertx vertx, String name, Router router, MiddlewareOptionsModel config) {
        ...
        final WithAuthHandlerMiddlewareOptionsBase options = castOptions(config,
            WithAuthHandlerMiddlewareOptionsBase.class);
        ...
        return JWTAuthPublicKeysReconcilerHandler.fetchPublicKeys(vertx, publicKeySources)
            .map(authOpts -> {
                ...
                final JWTAuthPublicKeysReconcilerHandler reconciler = JWTAuthPublicKeysReconcilerHandler.create(
                    vertx, jwtAuthOptions, additionalIssuersOptions, additionalClaimsOptions, publicKeySources,
                    publicKeysReconciliationEnabled, publicKeysReconciliationIntervalMs);
