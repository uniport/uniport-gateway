...
import static ch.uniport.gateway.proxy.middleware.openTelemetry.OpenTelemetryMiddleware.CONTEXTUAL_DATA_SESSION_ID;
 ...
 * The authentication can be required for the same scope (scope = "OIDC client
 * id") or for different scopes.
...
public class OAuth2AuthMiddleware extends TraceMiddleware {
    ...
    // prefix of the key for storing the oauth2 states in the session as a JSON
    // structure
    ...
    /**
     * Put the OAuth2 flow parameters back into the session for the state parameter
     * from the request.
     *
     * @param ctx
    ...
    protected static void registerCallbackHandlers(
        Vertx vertx, Route callback, String sessionScope,
    ...
    // this method is called when the IAM finishs the authentication flow and sends
    // a redirect (callback) with the code
    private static void whenAuthenticationResponseReceived(
        ...
        if (OAuth2AuthMiddleware.restoreStateParameterFromRequest(ctx, sessionScope)) {
            LOGGER.debug("processing for state '{}' and code '{}...'", stateParameter,
                code != null ? code.substring(0, 5) : null);
            ctx.addEndHandler(
                asyncResult -> whenTokenForCodeReceived(vertx, asyncResult, ctx, authProvider, sessionScope));
    ...
    // if the enhanced state parameter contains an uri, we send a redirect to it,
    // otherwise a status code 410 (GONE)
    // depending on the response_mode configuration the incoming request is GET or
    // POST
    private static void sendResponseFor(String stateParameter, RoutingContext ctx) {
        ...
        if (uri.isPresent()) {
            // fix PORTAL-1417: die HTTP Methode ist aufgrund response_mode=FORM_POST immer
            // POST, aber
    ...
    public void handleWithTraceSpan(RoutingContext ctx, Span span) {
        ...
        // synchronized: to prevent conflicts in writing the "state" value into the
        // session data
    ...
    /**
     * Update the RoutingContext with the user for the given sessionScope or clear
     * the user if not available.
     *
     * @param sessionScope
    ...
    private void startAndStorePendingAuth(RoutingContext ctx) {
        if (oAuth2FlowStarted(ctx)) {
            // create JSON object for authentication parameters and store in session at
            // "state_<state>"
