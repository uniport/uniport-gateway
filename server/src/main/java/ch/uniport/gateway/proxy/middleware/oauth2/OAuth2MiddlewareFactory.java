 ...
 *
 * Configures keycloak as the OAuth2 provider. It patches the authorization path
 * to ensure all
 * follow-up requests are routed through this application as well, if
 * configured.
 */
public class OAuth2MiddlewareFactory implements MiddlewareFactory {
    ...
    private static final Pattern ORIGIN_PATTERN = Pattern
        .compile("https?:\\/\\/[-a-zA-Z0-9@:%._\\+~#=]{1,256}\\.[a-zA-Z0-9()]{1,6}");
    ...
    public Future<Middleware> create(Vertx vertx, String name, Router router, MiddlewareOptionsModel config) {
        ...
        return KeycloakAuth.discover(vertx, oAuth2Options(options))
            .onFailure(err -> LOGGER.error(
                "Failed to create OAuth2 Middleware due to failing Keycloak discovery '{}'", err.getMessage()))
    ...
    /**
     * Additional request parameters for the OAuth 2.0 Authentication Request to the
     * Authorization Server.
     *
     * @see <a href=
     *      "https://openid.net/specs/openid-connect-core-1_0.html#AuthRequest">AuthRequest</a>
     */
    private Map<String, String> getAdditionalAuthRequestParams(
        AbstractOAuth2MiddlewareOptionsBase options,
        String responseMode
    ) {
    ...
    /**
     *
     * the protocol, hostname or port can be different from what the uniport-gateway
     * knows, therefore
     * in RouterFactory.createMiddleware the publicUrl configuration is added to
     * this middleware
     * configuration
     */
    ...
    private List<Route> mountCallbackRoutes(Router router, String callbackPath, String responseMode) {
        ...
        // PORTAL-2004: We always need a GET callback. In the case of FormPost, we need
        // the GET callback for requests with an Accept header that does not allow
        // text/html
        ...
        if (isFormPost(responseMode)) {
            // PORTAL-513: Forces the OIDC Provider to send the authorization code in the
            // body
            callbacks.add(router
                .post(callbackPath)
        ...
        return authProvider -> {
            ...
            if (callbackOrigin != null) {
                ...
                try {
                    ...
                    final String errMsg = String.format("Failed to parse callback origin '%s': %s", callbackOrigin,
                        err.getMessage());
                    LOGGER.warn(errMsg);
                    return Future.failedFuture(errMsg);
                }
                callbackURL = String.format("%s://%s%s", callbackOriginURI.getScheme(),
                    callbackOriginURI.getAuthority(), callbackPath);
            } else {
                callbackURL = String.format("%s%s", publicUrl, callbackPath);
            }

            // PORTAL-1184 we are using a patched OAuth2AuthHandlerImpl class as
            // OAuth2AuthHandler implementation.
    ...
    /**
     * By patching the issuer and the authorization path, ensure that Keycloak is
     * only visible from the outside as running behind the uniport-gateway.
     */
