...
/**
 * 
 * The CompositeCSPHandler is required to work a little bit different than the
 * CSPHandler by vertx-web.
 * 
 * First it needs to be able to union the CSP configuration of multiple
 * middlewares on incoming request
 * (The routing context should be used for trasmitting information between
 * middlewares).
 * 
 * Additionally, it is needs to do the same for middlewares on the incoming
 * response.
 * Hence, the handleResponse method.
 * 
 * One special case occurs for the first middleware on the incoming response:
 * The backend might send its own CSP, therefore the middleware needs to either
 * use
