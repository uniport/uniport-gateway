package ch.uniport.gateway.proxy.provider.file.JsonDirectoryConfigStore;
 ...
 * We only need to change the merge method in get(), but we cannot extend
 * DirectoryConfigStore and overwrite get(), because the path and filesets
 * members are
 * private
 * https://github.com/vert-x3/vertx-config/blob/4.5.16/vertx-config/src/main/java/io/vertx/config/impl/spi/DirectoryConfigStore.java#L95
 */
public final class JsonDirectoryConfigStore implements ConfigStore {
    ...
    // The difference is on how JsonArrays are merged. This implementation creates a
    // deduplicated concatenation of two arrays.
    // https://github.com/eclipse-vertx/vert.x/blob/4.5.16/src/main/java/io/vertx/core/json/JsonObject.java#L1007-L1026
