...
public class OAuth2RegistrationMiddlewareFactory extends OAuth2MiddlewareFactory {
    ...
    /**
     * Replaces the {@link #AUTH_ENDPOINT} substring with
     * {@link #REGISTRATION_ENDPOINT}.
     *
     * @param publicUrl
     *            the base url
     * @param keycloakAuthorizationEndpoint
     *            auth endpoint containing the patch to be
     *            patched.
     * @return the patched URL.
     */
    @Override
    protected String patchPath(String publicUrl, URI keycloakAuthorizationEndpoint) {
        final String registrationPath = keycloakAuthorizationEndpoint.getPath().replace(AUTH_ENDPOINT,
