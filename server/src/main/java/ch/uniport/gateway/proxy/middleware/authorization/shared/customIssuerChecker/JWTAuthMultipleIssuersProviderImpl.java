...
/**
 * https://github.com/eclipse-vertx/vertx-auth/blob/4.5.18/vertx-auth-jwt/src/main/java/io/vertx/ext/auth/jwt/impl/JWTAuthProviderImpl.java
 */
public class JWTAuthMultipleIssuersProviderImpl extends JWTAuthProviderImpl {
    ...
    public JWTAuthMultipleIssuersProviderImpl(
        Vertx vertx, JWTAuthOptions config,
        JWTAuthMultipleIssuersOptions options
    ) {
        super(vertx, config);
    ...
    public Future<User> authenticate(Credentials credentials) {
        // If we don't have multiple issuer, we can use the built-in vertx authenticate
        // check.
        if (additionalIssuers.isEmpty()) {
            return super.authenticate(credentials);
        }

        // Otherwise we manually check if the current jwt has one of our additional
        // issuer included.
        // Copied from the JWTAuthProviderImpl#authenticate method.
        // https://github.com/eclipse-vertx/vertx-auth/blob/4.5.18/vertx-auth-jwt/src/main/java/io/vertx/ext/auth/jwt/impl/JWTAuthProviderImpl.java#L146-L154
        ...
        // The jwt issuer is not one of the defined additional issuers, therefore we can
        // use the built-in check.
        if (!issuerFound) {
            return super.authenticate(credentials);
        }

        // The built-in check does not support multiple possible issuer.
        // Therefore, we need to bypass the built-in issuer check by setting the issuer
        // temporarily to null.
        // The built-in code for the check can be found at:
        // io.vertx.ext.auth.jwt.impl.JWTAuthProviderImpl, vertx-auth-jwt:4.3.8
