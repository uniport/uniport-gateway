...
import ch.uniport.gateway.proxy.config.label.Parser;
...
/**
 * Generates a complete dynamic configuration from announcements about
 * created/removed docker
 * containers.
 */
public class DockerContainerProvider extends Provider {
    ...
    private JsonObject filterExtraConfig(JsonObject labels) {
        ...
        final String dockerFilter = String.format("%s.%s", Parser.DEFAULT_ROOT_NAME,
    ...
    // If a container is linked to several networks and no network is specified,
    // then it will
    ...
    private JsonArray buildRouterConfiguration(JsonObject httpConf, Map<String, String> model) {
        ...
        for (int i = 0; i < routers.size(); i++) {
            ...
            for (int j = 0; j < middlewares.size(); j++) {
                final String middlewareName = middlewares.getJsonObject(j)
