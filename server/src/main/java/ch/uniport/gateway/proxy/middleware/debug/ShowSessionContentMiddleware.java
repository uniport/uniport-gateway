...
import static ch.uniport.gateway.proxy.middleware.oauth2.OAuth2AuthMiddleware.SINGLE_SIGN_ON_SID;
...
public class ShowSessionContentMiddleware extends TraceMiddleware {
    ...
    public void handleWithTraceSpan(RoutingContext ctx, Span span) {
        ...
        if (!ctx.request().absoluteURI().contains(ShowSessionContentMiddlewareFactory.TYPE)) {
            ctx.next();
            return;
        }
