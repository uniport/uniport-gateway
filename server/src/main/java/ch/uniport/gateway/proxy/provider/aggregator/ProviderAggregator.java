package ch.uniport.gateway.proxy.provider.aggregator;
...
public class ProviderAggregator extends Provider {
    ...
    public ProviderAggregator(
        Vertx vertx, String configurationAddress, List<ProviderModel> providerConfigs,
        JsonObject env
    ) {
        this.vertx = vertx;
    ...
    public void provide(Promise<Void> startPromise) {
        ...
        for (ProviderModel providerConfig : providerConfigs) {
            ...
            final Provider provider = providerFactory.get().create(this.vertx, this.configurationAddress,
