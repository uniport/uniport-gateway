 ...
 */
public final class RelyingPartyHandler extends HTTPAuthorizationHandler<OAuth2Auth>
        ...
        parseAuthorization(context, !bearerOnly, parseAuthorization -> {
            ...
            if (token == null) {
                if (isNoneInteractive(context.request())) {
                    ...
                    context.fail(401, new VertxException(
                        "OAuth2 No valid authentication in non-interactive request. Access Denied.", true));
                    return;
                }
                ...
                if (context.request().method() == HttpMethod.GET
                    ...
                    handler.handle(Future.failedFuture(
                        new HttpException(302, authURI(redirectUri, state, codeVerifier, context.request()))));
                }
            } else {
                // continue
    ...
    private String authURI(String redirectURL, String state, String codeVerifier, HttpServerRequest request) {
            ...
            for (Map.Entry<String, List<String>> query : queryParams.entrySet()) {
                if (passthroughParams.contains(query.getKey())) {
                    ...
                    if (query.getValue().size() > 1) {
                        ...
                        LOGGER.warn(
                            "Query parameter '{}' is supplied multiple times. Will use the first '{}' and ignore the rest",
                            query.getKey(), v);
                    }
                }
            }
        ...
        if (!acceptsTextHTML(request)) {
            // PORTAL-2004: change response_mode to `query` if `Accept:` header is not
            // `text/html`,
            // because JS code doesn't execute
            // `onload="javascript:document.forms[0].submit()"` trigger
        ...
        if (codeVerifier != null) {
            synchronized (sha256) {
                ...
                config
                    .putAdditionalParameter("code_challenge",
        ...
        for (Entry<String, Object> kv : extraParams.getMap().entrySet()) {
            ...
            if (!(v instanceof String)) {
                throw new IllegalArgumentException(
                    String.format("value for extra param '%s' must be a string", kv.getKey()));
            }
                    ...
                    for (String scope : this.scopes) {
                        ...
                        if (idx != -1) {
                            ...
                            if ((idx != 0 && handlerScopes.charAt(idx - 1) != ' ') ||
                                ...
                                ctx.fail(403, new VertxException(
                                    "principal scope != handler scopes. principal scope is " + scope, true));
                                return;
                            }
                        } else {
                            // invalid scope assignment
