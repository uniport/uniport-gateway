...
import ch.uniport.gateway.proxy.service.contextAware.ContextAwareHttpServerRequest;
...
public class ReverseProxy extends TraceMiddleware {
    ...
    public void handleWithTraceSpan(RoutingContext ctx, Span span) {
        ...
        LOGGER.debug("'{}' is proxying to '{}://{}:{}{}'", name, serverProto, serverHost, serverPort,
            ctx.request().uri());
        try {
    ...
    /**
     * Since version 4.3.5, the vertx-http-proxy sets the 'x-forwarded-host', in
     * case it detects that
     * the outgoing request has a different host header value than the incoming
     * request.
     * However, since version 4.5.15, it only checks if the authority was modified
     * in an interceptor, so
    ...
    /**
     * Generally, x-forwarded headers should only be added, if they are not yet
     * present on the request.
     * One exception is x-forwarded-for, that should be extended by each
     * intermediate proxy.
     * 
     * See
    ...
    /**
     * Generally, the requests or responses should be modified by the respective
     * middleware itself.
     * However, some operations are not possible on a HttpServerRequest, such as
     * changing the request's path.
     * Similarly for some operations on the response, like operations on response
     * headers.
            ...
            public Future<ProxyResponse> handleProxyRequest(ProxyContext proxyContext) {
                ...
                if (uriModifiers != null) {
                    if (uriModifiers.size() > 1) {
                        LOGGER.warn("Multiple URI modifiers declared: {} (total {})", uriModifiers,
                            uriModifiers.size());
                    }
    ...
    /**
     * Before handing the request to the vertx-http-proxy, we wrapped in into
     * {@code ContextAwareHttpServerRequest} to access
    ...
    private RoutingContext getContextFromRequest(ProxyContext proxyContext) {
        ...
        final ContextAwareHttpServerRequest contextAwareRequest = (ContextAwareHttpServerRequest) incomingRequest
            .proxiedRequest();
