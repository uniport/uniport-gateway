...
/**
 * Handle situations, where the browser has sent requests during the session id
 * regeneration.
 * These situations are detected by checking the session state and the value of
 * an additional cookie. If such
 * a situation is detected, the request is sent back as a redirect (=retry) to
 * the same URL but without returning a
 * new session id.
...
public class ReplacedSessionCookieDetectionMiddleware extends TraceMiddleware {
    ...
    /**
     * @return true, if an <strong>authenticated</strong> user's request is sent
     *         with the replaced session (but now invalid, due do
     *         session.regenerateId(),
     *         which has generated a new id for this session) <strong>session
     *         id</strong>. This special case is detected by checking (1) if the
     *         request contains
     *         our detection cookie (+ satisfies some conditions)
     *         ({@link #isDetectionCookieValueWithInLimit(RoutingContext)} and (2)
     *         if the user is not
     *         authenticated from the uniport-gateway's point of view (because the
     *         <strong>session id</strong> has changed)
     *         ({@link #noUserInSession(RoutingContext)}).
     *         <p>
     *         Detection-cookies are only handed to <strong>authenticated</strong>
     *         users on each new request (with
     *         {@link #responseWithDetectionCookie(RoutingContext)}).
     *         Therefore the detection-cookie serves as proof that the user has been
     ...
     * @param ctx
     *            the current routing context
     * @return true if no user is associated with this session: Either (a) the user
     *         has not been authenticated or (b) the session id has expired (due do
     *         session.regenerateId()).
     */
     ...
     *         Only authenticated users receive this cookie entry (see
     *         {@link #responseWithDetectionCookie(RoutingContext)})
     */
    ...
    /**
     * Sends a redirect (retry) response after a configurable delay. We expect/hope
     * that the browser has received the new session id in the meantime.
     * (Refer to the documentation for a visual explanation).
     * We also use the detection cookie to track how many requests were made with
     * the previously valid <strong>session id</strong>.
     */
    ...
    private void responseWithDetectionCookie(RoutingContext ctx) {
        if (isUserInSession(ctx)) {
            ...
            final Long sessionIdleTimeoutInMilliSeconds = ctx
                .get(SessionMiddleware.SESSION_MIDDLEWARE_IDLE_TIMEOUT_IN_MS_KEY);
            if (sessionIdleTimeoutInMilliSeconds == null) {
                throw new IllegalStateException("Session idle timeout is required");
            }
            setDetectionCookieTo(ctx.response(),
                Optional.of(new DetectionCookieValue(session.lastAccessed(), sessionIdleTimeoutInMilliSeconds)));
        }
    }
    ...
    /**
     * Reads the cookie from the request and returns the FIRST with the supplied
     * name.
     * 
     * This assumes there aren't multiple cookies with the same name (technically
     * possible due to the name/host/path tuple)
     * 
     * @param ctx
     * @param cookieName
     * @return
