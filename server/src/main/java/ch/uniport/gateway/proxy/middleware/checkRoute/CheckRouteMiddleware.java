 ...
 * If "_check-route_" is found within in the URI of the incoming request, the
 * request is
 * ended immediately and returns a status code 202. Otherwise, the request is
 * passed on.
 */
public class CheckRouteMiddleware extends TraceMiddleware {
    ...
    private boolean isCheckRoute(RoutingContext ctx) {
        return ctx.request().uri().contains(CheckRouteMiddlewareFactory.PATH);
    }
