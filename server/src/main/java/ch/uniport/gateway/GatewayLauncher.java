...
import ch.uniport.gateway.core.GatewayVerticle;
...
public class GatewayLauncher extends Launcher {
    ...
    public void beforeStartingVertx(VertxOptions options) {
        ...
        if (Runtime.isDevelopment()) {
            // increase the max event loop time to 10 min (default is 2_000_000_000 ns = 2s)
            // to omit thread blocking warnings
    ...
    private Config configureClusterManager() {
        final String headlessServiceName = System.getenv().getOrDefault(HEADLESS_SERVICE_NAME_PROPERTY,
            DEFAULT_HEADLESS_SERVICE_NAME);

        // See https://vertx.io/docs/vertx-hazelcast/java/#_configuring_for_kubernetes
        // See
        // https://docs.hazelcast.com/hazelcast/latest/kubernetes/kubernetes-auto-discovery#using-kubernetes-in-dns-lookup-mode
