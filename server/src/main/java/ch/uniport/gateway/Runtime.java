...
public final class Runtime {
    ...
    /**
     * Development mode can be activated by setting the environment variable
     * 'DEVELOPMENT'.
     *
    ...
    /**
     * The logback.xml for the logback configuration is taken from one of these
     * places:
     ...
     * 3. File 'logback.xml' in '/etc/uniport-gateway', if
     * UNIPORT_GATEWAY_STRUCTURAL_LOGGING_ENABLED=true
     * 4. File 'logback-unstructured.xml' in '/etc/uniport-gateway', if
     * UNIPORT_GATEWAY_STRUCTURAL_LOGGING_ENABLED=false
    ...
    public static Optional<Path> getLoggingConfigPath() {
        ...
        if (existsAsFile(loggingConfigFilePath)) {
            ...
            if (loggingConfigFilePath != null) {
                System.err.println("File '" + new File(loggingConfigFilePath).getAbsoluteFile() + "' for system env '"
                    + LOGGING_CONFIG_PROPERTY + "' not found");
            }
        }
        ...
        if (existsAsFile(loggingConfigFilePath)) {
            ...
            if (loggingConfigFilePath != null) {
                System.err.println("File '" + new File(loggingConfigFilePath).getAbsoluteFile()
                    + "' for system property '" + LOGGING_CONFIG_PROPERTY + "' not found");
            }
        }
