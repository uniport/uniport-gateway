...
public class ParserTest {
    ...
    static Stream<Arguments> filterKeysTestData() {
        ...
        return Stream.of(Arguments.of("labels is null", null, null, List.of()),
                Arguments.of("identity is returned if filter is null", labels, null,
                ...
                Arguments.of("filter keys", labels, List.of("test", "some"),
                        List.of("testA", "someB")));
    }

    @ParameterizedTest
    @MethodSource("filterKeysTestData")
    void filterKeysTest(String name, Map<String, Object> labels, List<String> filters,
            List<String> expected, Vertx vertx, VertxTestContext testCtx) {
        String errMsg =
                String.format("'%s' failed. Labels: '%s', Filters: '%s'", name, labels, filters);
    ...
    static Stream<Arguments> decodeTestData() {
        ...
        Map<String, Object> labels = new HashMap<String, Object>(Map.ofEntries(//
                new AbstractMap.SimpleEntry<String, Object>("test.http.routers.blub.rule",
                        "someRule"), //
                new AbstractMap.SimpleEntry<String, Object>("test.http.routers.blub.middlewares",
                        "one, two,foo"), //
                new AbstractMap.SimpleEntry<String, Object>("test.http.middlewares.foo.headers.bar",
                        "baz"), //
                ...
                put(DynamicConfiguration.HTTP, new JsonObject()//
                        .put(DynamicConfiguration.ROUTERS, new JsonArray()//
                                .add(new JsonObject()//
                                        ...
                                        .put(DynamicConfiguration.ROUTER_MIDDLEWARES,
                                                new JsonArray()//
                                                        .add("one")//
                                                        .add("two")//
                                                        .add("foo"))))
    ...
    void decodeTest(String name, Map<String, Object> labels, String rootName, JsonObject expected,
            Vertx vertx, VertxTestContext testCtx) {
        List<String> filters = null; // usage of filters is tested in 'filterKeysTest'
        String errMsg = String.format("'%s' failed. Labels: '%s', RootName: '%s', Filters: '%s'",
                name, labels, rootName, filters);
