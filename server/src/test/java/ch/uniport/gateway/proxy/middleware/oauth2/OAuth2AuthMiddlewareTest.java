...
public class OAuth2AuthMiddlewareTest extends MiddlewareTestBase {
    ...
    protected Stream<Arguments> provideConfigValidationTestData() {
        ...
        return Stream.of(
            ...
            Arguments.of("reject config with missing required property", missingRequiredProperty, complete,
                expectedFalse),
    ...
    @Test
    void redirectForAuthenticationRequestWithAcceptAll(Vertx vertx, VertxTestContext testCtx)
    ...
    void redirectForAuthenticationRequestHasResponseModeFormPost(Vertx vertx, VertxTestContext testCtx)
        ...
        final MiddlewareServer gateway = MiddlewareServerBuilder.portalGateway(vertx, testCtx)
            ...
            .build()
            .start();
        final RequestOptions reqOpts = new RequestOptions().addHeader(HttpHeaders.ACCEPT,
    ...
    @Test
    void redirectForAuthenticationRequestRefusingTextHtml(Vertx vertx, VertxTestContext testCtx)
        throws InterruptedException {
        // given
        final KeycloakServer keycloakServer = new KeycloakServer(vertx, testCtx, "localhost")
            .startWithDefaultDiscoveryHandler();
    ...
    @Test
    void redirectForAuthenticationRequestWithMultipleAcceptHeaders(Vertx vertx, VertxTestContext testCtx)
        throws InterruptedException {
        // given
        final KeycloakServer keycloakServer = new KeycloakServer(vertx, testCtx, "localhost")
            .startWithDefaultDiscoveryHandler();
    ...
    void redirectForAuthenticationRequestWithPassthroughParam(
        List<String> passthrough, Map<String, List<String>> in,
        Map<String, String> expected, Vertx vertx, VertxTestContext testCtx
    ) throws Throwable {
        // given
        final KeycloakServer keycloakServer = new KeycloakServer(vertx, testCtx, "localhost")
            .startWithDefaultDiscoveryHandler();
    ...
    void callbackRequestWithErrorLoginRequired(Vertx vertx, VertxTestContext testCtx) throws InterruptedException {
        ...
        // when
        gateway.incomingRequest(POST,
            "/callback/test?error=login_required&state=" + oidcSessionState.getString(OIDC_PARAM_STATE)
                + "&code=ghijklmnop",
            outgoingResponse -> {
                // then
                assertThat(testCtx, outgoingResponse)
    ...
    // Dieser Test überprüft, dass ein Code Callback Request mit ungültigem Session
    // Cookie
    ...
    // Dieser Test überprüft, dass ein Code Callback Request mit ungültigem Session
    // Cookie
    // zu keinem Hänger (PORTAL-1184) führt, sondern Status Code `307` zurückgibt,
    // falls im state Parameter enthalten.
    @Test
        ...
        gateway.incomingRequest(GET, "/", (outgoingResponse) -> {
            ...
            gateway.incomingRequest(POST,
                ...
                httpClientResponse -> {
                    // then
                    assertThat(testCtx, httpClientResponse)
                        .hasStatusCode(HttpResponseStatus.INTERNAL_SERVER_ERROR.code()); // Keycloak serves a
                                                                                                                                // 400, but the Oauth2
                                                                                                                                // middleware will
                                                                                                                                // convert this into a
                                                                                                                                // 500
    ...
    void authorizationPathShouldBePatched(Vertx vertx, VertxTestContext testCtx) throws Throwable {
        ...
        gateway.incomingRequest(GET, "http://localhost:8080", reqOpts, (outgoingResponse) -> {
            // then
            assertThat(testCtx, outgoingResponse)
                .isRedirectToWithoutParameters(publicUrl + "/auth/realms/test") // redirect path to OP should be
                // patched
                .isValidAuthenticationRequest(Map.of(
    ...
    void callbackPathShouldBePatched(Vertx vertx, VertxTestContext testCtx) throws Throwable {
        ...
        gateway.incomingRequest(GET, "http://localhost:8080", reqOpts, (outgoingResponse) -> {
            // then
            assertThat(testCtx, outgoingResponse)
                .isRedirectToWithoutParameters(
                    "http://" + keycloakServer.host() + ":" + keycloakServer.port() + "/auth/realms/test") // redirect
                // should
                // point
                // directly
                // to
                // Keycloak
                .isValidAuthenticationRequest(Map.of(
                    "redirect_uri", publicUrl + "/callback/" + scope, // callback should still point to the
                    // gateway
                    "scope", "openid test"))
                .isUsingFormPost()
                .hasPKCE();
    ...
    void callbackOriginShouldBeConfigurable(Vertx vertx, VertxTestContext testCtx) throws Throwable {
        ...
        gateway.incomingRequest(GET, "http://localhost:8080", reqOpts, (outgoingResponse) -> {
            ...
            assertThat(testCtx, outgoingResponse)
                ...
                // Keycloak
                .isValidAuthenticationRequest(Map.of(
                    "redirect_uri", callbackOrigin + "/callback/" + scope, // callback should point to the
                    // configured origin
                    "scope", "openid test"))
                .isUsingFormPost()
                .hasPKCE();
    ...
    void fixedPORTAL1184(Vertx vertx, VertxTestContext testCtx) throws InterruptedException {
        ...
        gateway.incomingRequest(POST,
            "/callback/protected1?state=" + oidcSessionState.getString(OIDC_PARAM_STATE) + "&code=THE-CODE",
            response1 -> {
                ...
                gateway.incomingRequest(GET, protectedResource2, withCookie(reqOpts, sessionCookies.get()),
                    (response2) -> {
                        ...
                        gateway.incomingRequest(GET, protectedResource3,
                            withCookie(reqOpts, sessionCookies.get()),
                            (response3) -> {
                                ...
                                // 4: callback for auth flow 2 -> code-to-token -> session with regenerated
                                // id
                                gateway.incomingRequest(POST,
                                    "/callback/protected2?state=" + state2 + "&code=THE-CODE",
                                    withCookie(reqOpts, sessionCookies.get()),
                                    (response4) -> {
                                        ...
                                        // 5:
                                        gateway.incomingRequest(POST,
                                            "/callback/protected2?state=" + state3
                                                + "&code=THE-CODE",
                                            withCookie(reqOpts, sessionCookies.get()),
                                            (response5) -> {
                                                // then
