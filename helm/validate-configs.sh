...
function validateWithDynamicSchema() {
  docker run --rm \
    ...
    -v "${PWD}":/tmp \
    $NODE_IMAGE \
    npx ajv-cli validate --allow-union-types -s /tmp/target/schema/portalGatewayDynamicSchema.json -d "/tmp/target/portal-gateway/proxy-config.*/dynamic-config/**/*.json"
}
...
function validateWithStaticSchema() {
  docker run --rm \
    ...
    -v "${PWD}":/tmp \
    $NODE_IMAGE \
    npx ajv-cli validate --allow-union-types -s /tmp/target/schema/portalGatewayStaticSchema.json -d "/tmp/target/portal-gateway/proxy-config.*/*.json"
}
